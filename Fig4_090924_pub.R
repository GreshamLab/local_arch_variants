...
# import clone data
all_clones = read_csv("Supplementary_File_2_clone_sequencing_analysis_092424.csv")
...
ars_clones = all_clones %>% filter(Description=="ARS KO") %>% arrange(cnv_length) %>% 
mutate(group = case_when(cnv_length <= 8000 ~ "smaller CNV",
                          cnv_length > 8000 ~ "larger CNV"))

with(ars_clones, table(CNV_Mechanism, group)) %>% as.data.frame() %>% 
  group_by(group) %>% 
  mutate(proportion_geno = Freq/sum(Freq)) %>% ungroup() %>%
  mutate(CNV_Mechanism = fct_relevel(CNV_Mechanism, "NAHR","complex CNV", "LTR NAHR", "ODIRA" )) %>% #custom order
  ggplot(aes(y=CNV_Mechanism, x = proportion_geno, fill = group)) +
...
# ggsave("ars_mechs_plot.png") #Saving 6.58 x 4.94 in image
# ggsave("ars_mechs_plot.pdf") #Saving 6.58 x 4.94 in image
...
# ggsave("HorizontalBarProp_cnvMechs_Genotype_091224.png", width = 8.5, height = 7, bg = "white")
# ggsave("HorizontalBarProp_cnvMechs_Genotype_091224.pdf", width = 8.5, height = 7, bg = "white")
...
ggsave("scatterplot_Alpha_Label_091824.png", width = 12, height = 8, bg = "white")
ggsave("scatterplot_Alpha_091824.pdf", width = 12, height = 8, bg = "white")
all_clones %>%
...
#all_clones %>% filter(ODIRAtype %in% c("ODIRA one end", "ODIRA", "ODIRA no ARS")) %>% write_csv("allclones_ODIRA_092024.csv")
...
# Further investigate ODIRA clones that have 2 copies ##
all_clones %>%
  filter(CNV_Mechanism=="ODIRA" & gap1_rounded == 2) %>% View()

#ODIRA clones that end in TEL11
all_clones %>%
  filter(CNV_Mechanism=="ODIRA" & CNV_breakpoint_down == "TEL11") %>% View()

#ODIRA no ARS
all_clones %>% 
  filter(ODIRAtype=="ODIRA no ARS") %>% View()
  
  
# Supplementary Table 3 - Count of Mechanism separated by strain ####
with(all_clones,table(Description, CNV_Mechanism)) %>% as.data.frame() %>%
  pivot_wider(names_from = Description, CNV_Mechanism, values_from = Freq) %>% 
  mutate(total = rowSums(.[, 2:5])) %>%
  adorn_totals("row")

# Supplementary Figure S4D - Interaction between CNV Length, Sc, CNV formation rate.
all_clones = all_clones %>% 
  mutate(SelCoeff = case_when(Description == "Wildtype architecture" ~ 0.182,
                              Description == "LTR KO" ~ 0.182,
                              Description == "LTR and ARS KO" ~ 0.126,
                              Description == "ARS KO" ~ 0.146
                              ),
         FormRate = case_when(Description == "Wildtype architecture" ~ 4.5e-5,
                              Description == "LTR KO" ~ 1e-5,
                              Description == "LTR and ARS KO" ~ 1e-5,
                              Description == "ARS KO" ~ 2.4e-6
                              )
        )

summary(aov(cnv_length~Description*FormRate, data = all_clones))
lm(cnv_length ~ Description + SelCoeff + FormRate, data=all_clones)

model <- aov(cnv_length ~ SelCoeff * FormRate, data=all_clones)
summary(model)
# no signifcant effect on SelCoeff or FormRate on Cnv Length. 
# Significant interaction between SelCoeff and FormRate on CNV Length.
#                     Df    Sum Sq   Mean Sq F value   Pr(>F)    
# SelCoeff            1 2.818e+09 2.818e+09   0.179 0.673014    
# FormRate            1 1.875e+10 1.875e+10   1.189 0.277029    
# SelCoeff:FormRate   1 1.881e+11 1.881e+11  11.925 0.000696 ***
# Residuals         173 2.728e+12 1.577e+10  

model <- aov(cnv_length ~ Description, data=all_clones)
summary(model)


# list.files()
# chemoNums = read_csv("samplesheet_chemoNumbers_092424.csv")
# head(chemoNums)
# head(all_clones)
# full_join(all_clones, chemoNums) %>% 
#   relocate(chemostat, .after = pop_name) %>% 
#   write_csv("clone_sequencing_analysis_092424.csv")
